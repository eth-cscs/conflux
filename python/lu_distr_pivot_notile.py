...
def LU_rep(A, measureComm, commCounter, commRcvCounter):
    ...
    # holds global row indices used in tournament pivoting. 2*v indices: v for local
    # candidate rows, another v for neighbor candidate rows
    growsBuff = np.zeros([P, v * max(2, tA11)])
...
# now k is a step number
    for k in range(Nt):
        ...
        tmp = A11Buff[3, A11MaskBuff[3]]
        checksum = 0
        for pp in range(P):
            [pi, pj, pk] = p2X(pp)
            if pj == k % sqrtp1:
                checksum += np.linalg.norm(A11Buff[pp, A11MaskBuff[pp], loff:])
            else:
        ...
        TournPivotNoTile(k, PivotBuff, A11Buff, A00Buff, A11MaskBuff, growsBuff, curPivots, curPivOrder,
                             pivotIndsBuff, layrK, measureComm, commCounter, commRcvCounter)
        ...
        for p in range(P):
            ...
            for pivInd in range(1, curPivots[p, 0] + 1):
                pivot = curPivots[p, pivInd]
                if k == 2 and p_rcv == 2:
                    a = 1
                A11Buff[p_rcv, pivot, loff:] += A11Buff[p, pivot, loff:]
                # A11Buff[p_rcv, pivot, loff + v: ] += A11Buff[p, pivot, loff + v: ]

                # --- comm counters --- #
                if measureComm:
                    if p_rcv != p:
                        data_size = np.size(A11Buff[p, pivot, loff + v: ])
                        commCounter[p_rcv] += data_size
            ...
            for i in range(curPivots[p,0]):
                ...
                if measureComm:
                    if p_rcv != p:
                        ...
                        commCounter[p_rcv] += data_size
                        commRcvCounter[p_rcv, 3] += data_size
                # - end comm counters - #
        ...
        for p in range(P):
            ...
            rows = A11MaskBuff[p]
            # this is basically a sparse-dense A10 = A10 * U^(-1)   (BLAS triangular solve) with A10 sparse and U dense
            if (k ==2  and p == 3):
                a = 1;
        ...
        for p in range(P):
            ...
            if k == 2 and p == 2:
                a = 1
            A01Buff[p, :, loff: ] = trsm(A00L, A01Buff[p, :, loff: ], lower = True)
            ...
            for pk_rcv in range(c):
                ...
                for pi_rcv in range(sqrtp1):
                    ...
                    if measureComm:
                        if p_rcv != p:
                            data_size = np.size(A01Buff[p, rowStart:rowEnd, loff:])
                            commCounter[p_rcv] += data_size
        ...
        # ---------------------------------------------- #
        if k == 2:
            tmp2 = A10BuffRcv[2, A11MaskBuff[2]]
            tmp3 = A01BuffRcv[2]
            a = 1
