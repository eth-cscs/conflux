...
void push_pivots_up(std::vector<T>& in, std::vector<T>& temp,
    ...
    // map pivots to bottom rows (after non_pivot_rows)
    for (int i = 0; i < curPivots[0]; ++i) {
        int pivot_row = first_non_pivot_row + l2c[curPivots[i+1]];
    ...
    for (int i = first_non_pivot_row; i < first_non_pivot_row + curPivots[0]; ++i) {
        if (!pivots[i]) {
    ...
    for (int i = first_non_pivot_row + curPivots[0]; i < n_rows; ++i) {
        if (pivots[i]) {
...
*/
    ...
    for (auto k = 0; k < Nt; ++k) {
        ...
        */
        ...
        for (int i = 0; i < curPivots[0]; ++i) {
            std::iter_swap(&l2c[curPivots[i+1]], &l2c[i]);
        }
        ...
        if (pk == layrK) {
            ...
            for (int i = 0; i < curPivots[0]; ++i) {
                ...
                MPI_Put(&A01Buff[0], Nl-loff, MPI_DOUBLE,
...
#pragma omp parallel for
        ...
        if (pj == k % sqrtp1 && pk == layrK) {
            ...
            for (int i = 0; i < n_local_active_rows; ++i) {
                // l2c  Nl -> n_local_active_rows
                int grow = l2c[i];
                std::copy_n(&A10Buff[i * v], v, &A11Buff[grow*Nl+loff]);
            }
        }
        ...
        if (pi == k % sqrtp1 && pk == layrK) {
            for (int i = 0; i < curPivots[0]; ++i) {
                ...
                std::copy_n(&A01Buff[i * (Nl-loff)],
                            Nl,
                            &A11Buff[pivot_row * Nl + loff + v]);
            }
        }
